...
const Game: React.FC<GameProps> = () => {
    ...
    const yourHand = useAppSelector((state) =>
        selectPlayerHand(state, YOUR_PLAYER_ID)
    )
    const activePlayerId =
        playerHovered !== null ? playerHovered.id : YOUR_PLAYER_ID
    ...
    const currentPlayer = useAppSelector((state) =>
        selectPlayerById(state, turnOfPlayerId)
    )
    const you = useAppSelector((state) =>
        selectPlayerById(state, YOUR_PLAYER_ID)
    )
    ...
    const onShowStatsClick = useCallback(() => {
        dispatch(showStats())
    }, [dispatch])
    ...
    useEffect(() => {
        ...
        if (turnOfPlayerId !== YOUR_PLAYER_ID) {
            if (cardToTake) {
                ...
                timer = setTimeout(() => {
                    dispatch(
                        takeCard({ playerId: turnOfPlayerId, card: cardToTake })
                    )
                }, TURN_TIME_MS / 2)
            } else {
                timer = setTimeout(() => {
        ...
        <div className="game-page Page">
                ...
                <div className="game-players-board">
                    ...
                    {boards.map(({ playerId, board }) => (
                        <GameBoard
                            key={playerId}
                            cards={board}
                            isVisible={playerId === activePlayerId}
                        />
                    ))}
                </div>
                <div className="game-hand-wrapper">
                    ...
                    <div className="game-hand" ref={gameHandRef}>
                        <div className="game-hand-inner">
                            {((playerHovered &&
                                playerHovered.id !== YOUR_PLAYER_ID) ||
                                isGameOver) && (
                                <div className="game-hand-overlay"></div>
                            )}
                            {yourHand.map((card) => (
                                <Card
                                    key={card.id}
                                    ...
                                    onClick={onCardClick}
                                    status={
                                        currentPlayer.balance >= card.cost
                                            ? 'available'
                                            : 'not-available'
                                    }
                                />
            ...
            <div className="game-right">
                <div className="game-stats">
                    <div className="game-stat game-global-pollution">
                        Pollution Globale
                        <div className="number">{globalPollution}</div>
                    </div>
                    <div className="game-stat game-your-balance">
                        Votre Argent<div className="number">{you.balance}</div>
                    </div>
                    <div className="game-stat game-your-score">
                        Votre Score<div className="number">{you.score}</div>
                    </div>
                </div>
                {!isGameOver && (
                ...
                {isGameOver && (
                    <div className="game-end-turn" onClick={onShowStatsClick}>

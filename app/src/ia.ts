...
export const useBuyCardsAI = (player: Player): Card | null => {
    const globalPollution = useAppSelector(selectGlobalPollution);

    // Memoize affordable cards calculation
    const affordableCards = useMemo(() => _affordableCards(player), [player.hand, player.balance]);

    // Memoize the sorting logic
    const selectedCard = useMemo(() => {
        if (affordableCards.length === 0 || player.id === YOUR_PLAYER_ID) return null;
    ...
    }, [affordableCards, player.strategy, player.board, globalPollution]);
